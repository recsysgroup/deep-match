...
export TRAIN_NEG_TABLE=odps://etao_backend/tables/buyershow_dataset_neg_sample_v5
...
-DuserDefinedParameters="--task_type=train --train_max_step=500000 --learning_rate=1e-3 --train_table='${TRAIN_TABLE}' --eval_table='${TRAIN_TABLE}' --train_match_table='${TRAIN_MATCH_TABLE}' --eval_match_table='${TRAIN_MATCH_TABLE}' --train_neg_table='${TRAIN_NEG_TABLE}' --eval_neg_table='${EVAL_NEG_TABLE}' --model_dir=oss://jiusheng-tmp/fm_model_v7 --tmp_dir=oss://jiusheng-tmp/fm_tmp --train_batch_size=128"
'